...
// ===== STORAGE KEYS =====
var STORAGE="limebay_v14";var SHIFT_KEY="limebay_shift7";var ZONE_KEY="limebay_zone";
var HIST_KEY="limebay_hist2";var LEARN_KEY="limebay_learn";var CHURN_KEY="limebay_churn";
...
var arrivalSpread=0;
var bikeChurn={};
var learnData=JSON.parse(localStorage.getItem(LEARN_KEY)||"{}");
var prevBikePositions={};
...
// ===== BIKE COUNTING (nearest-bay assignment) =====
...
// ===== BIKE SPREAD (how scattered bikes are around bay) =====
function bikeSpread(bay){
  var positions=[];
  bMarks.forEach(function(mk){
    var l=mk.getLatLng();
    if(dist(bay.lat,bay.lng,l.lat,l.lng)<=zData.radius){positions.push([l.lat,l.lng])}
  });
  if(positions.length<2)return 0;
  var maxD=0;
  for(var i=0;i<positions.length;i++){
    for(var j=i+1;j<positions.length;j++){
      var d=dist(positions[i][0],positions[i][1],positions[j][0],positions[j][1]);
      if(d>maxD)maxD=d;
    }
  }
...
function trackChurn(){
  bays.forEach(function(bay){
    var key=bay.name;
    var bc=bikesAt(bay);
    if(!bikeChurn[key]){bikeChurn[key]={prev:bc,changes:0,checks:0,refills:0,lastTidy:0,refilledAfterTidy:false};return}
    var delta=Math.abs(bc-bikeChurn[key].prev);
    bikeChurn[key].changes+=delta;
    bikeChurn[key].checks++;
    if(bikeChurn[key].lastTidy>0&&bc>3&&(Date.now()-bikeChurn[key].lastTidy)<600000){
      bikeChurn[key].refilledAfterTidy=true;
      bikeChurn[key].refills++;
    }
...
function isBlackHole(bay){var ck=bikeChurn[bay.name];return ck&&ck.refills>=2}

// ===== LEARNING MODEL =====
function recordVisit(bayName,duration,bikesOnArrival,spread,scooterCount,hourOfDay,dayOfWeek,refilled){
  if(!learnData[bayName])learnData[bayName]={visits:[],avgTime:5,timePerBike:1,baseTime:2};
  var visit={dur:duration,bikes:bikesOnArrival,spread:spread,scooters:scooterCount,hour:hourOfDay,day:dayOfWeek,refilled:refilled,ts:Date.now()};
  learnData[bayName].visits.push(visit);
  if(learnData[bayName].visits.length>50)learnData[bayName].visits=learnData[bayName].visits.slice(-50);
  recalcModel(bayName);
  localStorage.setItem(LEARN_KEY,JSON.stringify(learnData));
}

function recalcModel(bayName){
  var d=learnData[bayName];if(!d||d.visits.length<1)return;
  var visits=d.visits;
  var totalTime=0;var totalBikes=0;
  visits.forEach(function(v){totalTime+=v.dur;totalBikes+=v.bikes});
  d.avgTime=Math.round(totalTime/visits.length*10)/10;
  if(totalBikes>0){
    d.timePerBike=Math.round(totalTime/totalBikes*10)/10;
  }
  d.baseTime=Math.max(1,d.avgTime-(d.timePerBike*(totalBikes/visits.length)));
  // Hour-of-day adjustments
  d.hourAdj={};
  visits.forEach(function(v){
    if(!d.hourAdj[v.hour])d.hourAdj[v.hour]={total:0,count:0};
    d.hourAdj[v.hour].total+=v.dur;
    d.hourAdj[v.hour].count++;
  });
  // Spread factor
  var spreadVisits=visits.filter(function(v){return v.spread>0});
  if(spreadVisits.length>2){
    var highSpread=spreadVisits.filter(function(v){return v.spread>10});
    var lowSpread=spreadVisits.filter(function(v){return v.spread<=10});
    if(highSpread.length>0&&lowSpread.length>0){
      var avgHigh=highSpread.reduce(function(s,v){return s+v.dur},0)/highSpread.length;
      var avgLow=lowSpread.reduce(function(s,v){return s+v.dur},0)/lowSpread.length;
      d.spreadPenalty=Math.max(0,Math.round((avgHigh-avgLow)*10)/10);
    }
  }
}

function predictTime(bay){
  var d=learnData[bay.name];
  var bc=bikesAt(bay);
  var spread=bikeSpread(bay);
  var hour=new Date().getHours();
  if(!d||d.visits.length<1){
    return Math.max(2,Math.round(2+bc*0.8));
  }
  var predicted=d.baseTime+(bc*d.timePerBike);
  if(d.hourAdj&&d.hourAdj[hour]){
    var hourAvg=d.hourAdj[hour].total/d.hourAdj[hour].count;
    predicted=(predicted+hourAvg)/2;
  }
  if(d.spreadPenalty&&spread>10){
    predicted+=d.spreadPenalty;
  }
  var ck=bikeChurn[bay.name];
  if(ck&&ck.refilledAfterTidy){predicted*=1.3}
  return Math.max(1,Math.round(predicted*10)/10);
}

function getEfficiency(bay){
  var pt=predictTime(bay);
  var walkMin=Math.ceil(dist(uLat,uLng,bay.lat,bay.lng)/80);
  var totalCost=walkMin+pt;
  return totalCost>0?Math.round(1/totalCost*1000)/10:0;
}
...
function openTidy(i){
  ...
  arrivalBikes=bikesAt(bays[i]);
  arrivalSpread=bikeSpread(bays[i]);
  document.getElementById("tidyTitle").textContent=bays[i].name;
  document.getElementById("tidyLabel").textContent=bays[i].red?"Red star bay | "+arrivalBikes+" bikes":"Bay | "+arrivalBikes+" bikes";
  var pt=predictTime(bays[i]);
  var visits=learnData[bays[i].name]?learnData[bays[i].name].visits.length:0;
  document.getElementById("tidyPrediction").textContent=visits>0?"Predicted: ~"+pt+" min (based on "+visits+" visits)":"No data yet - timing this visit";
  oP("tidyPopup");
}
...
function confirmTidy(s){
  ...
  var b=bays[pendTidy];
  var duration=Math.round((Date.now()-arrivalTime)/60000*10)/10;
  b.ts=s;b.at=Date.now();
  // Record learning data
  if(s!=="skipped"&&duration>0.1&&duration<60){
    var scooters=0;
    bMarks.forEach(function(mk){var l=mk.getLatLng();if(dist(b.lat,b.lng,l.lat,l.lng)<=zData.radius)scooters++});
    var now=new Date();
    var refilled=bikeChurn[b.name]&&bikeChurn[b.name].refilledAfterTidy;
    recordVisit(b.name,duration,arrivalBikes,arrivalSpread,scooters,now.getHours(),now.getDay(),refilled);
    if(bikeChurn[b.name])bikeChurn[b.name].lastTidy=Date.now();
  }
  if(s!=="skipped"){
  ...
  save();renderBays();cP("tidyPopup");pendTidy=null;arrivalTime=null;
}
...
function renderBays(){
  ...
  bays.forEach(function(bay,i){
    var bc=bikesAt(bay);var overCap=bc>zData.maxCap;var messy=isMessy(bay);var bh=isBlackHole(bay);
    var ic=iR;
...
setInterval(function(){refreshOnly()},300000);
...
function nearestBay(){
  ...
  toast(n.name+" - "+fmtDist(m)+" ~"+Math.ceil(m/80)+"min walk");
}

// ===== SMART ROUTE (efficiency-scored, churn-aware, time-predicted) =====
function buildClusters(items){
  ...
  items.forEach(function(item,i){
    ...
    var totalPredicted=cluster.reduce(function(s,c){return s+c.predicted},0);
    clusters.push({items:cluster,lat:cLat,lng:cLng,totalBikes:cluster.reduce(function(s,c){return s+c.bikes},0),totalTime:totalPredicted});
  });
  return clusters;
}
...
function showRoute(){
  var active=[];
  bays.forEach(function(b,i){if(b.ts)return;var bc=bikesAt(b);if(bc>0){
    var pt=predictTime(b);var bh=isBlackHole(b);var messy=isMessy(b);var overCap=bc>zData.maxCap;
    active.push({bay:b,idx:i,bikes:bc,predicted:pt,dist:0,walkMin:0,overCap:overCap,messy:messy,blackHole:bh,efficiency:0});
  }});
  ...
  while(remC.length>0){
    ...
    for(var k=0;k<remC.length;k++){
      ...
      var walkMin=Math.ceil(d2/80);
      var clusterEfficiency=remC[k].items.length/(walkMin+remC[k].totalTime)*60;
      var bonus=0;
      if(remC[k].items.some(function(x){return x.overCap}))bonus-=500;
      var nextChk=getNextCheck();
      if(nextChk&&nextChk.minsLeft<=30){remC[k].items.forEach(function(x){if(nextChk.sched.bays.indexOf(x.bay.name)!==-1)bonus-=1000})}
      if(remC[k].items.every(function(x){return x.blackHole}))bonus+=800;
      var score=d2-(clusterEfficiency*50)-(remC[k].totalBikes*10)+bonus;
  ...
  orderedClusters.forEach(function(cl){
    ...
    while(rem.length>0){
      ...
      p.efficiency=p.walkMin+p.predicted>0?Math.round(1/(p.walkMin+p.predicted)*600)/10:0;
      fullRoute.push(p);rLat=p.bay.lat;rLng=p.bay.lng;
    }
  });

  var totalDist=fullRoute.reduce(function(s,r){return s+r.dist},0);
  var totalPredicted=fullRoute.reduce(function(s,r){return s+r.predicted+r.walkMin},0);
  var overCount=fullRoute.filter(function(r){return r.overCap}).length;
  var bhCount=fullRoute.filter(function(r){return r.blackHole}).length;
  var predictedBph=totalPredicted>0?Math.round(fullRoute.length/(totalPredicted/60)*10)/10:0;

  var info=fullRoute.length+" bays | ~"+Math.round(totalPredicted)+" min total | ~"+fmtDist(totalDist)+" walking";
  info+="<br><span style='color:#32D74B;font-weight:700'>Predicted: "+predictedBph+" bph</span>";
  if(overCount)info+=" | <span style='color:#FF453A'>"+overCount+" overcap</span>";
  if(bhCount)info+=" | <span style='color:#FF9F0A'>"+bhCount+" black holes</span>";
  document.getElementById("routeInfo").innerHTML=info;

  // Skip suggestion
  var suggestion="";
  var bhBays=fullRoute.filter(function(r){return r.blackHole});
  if(bhBays.length>0){
    var bhTime=bhBays.reduce(function(s,r){return s+r.predicted+r.walkMin},0);
    var withoutBh=fullRoute.length-bhBays.length;
    var withoutBhTime=totalPredicted-bhTime;
    var altBph=withoutBhTime>0?Math.round(withoutBh/(withoutBhTime/60)*10)/10:0;
    if(altBph>predictedBph){
      suggestion="<div class='insight'><h4>Skip Suggestion</h4><p>Skip "+bhBays.map(function(r){return r.bay.name}).join(", ")+" (black holes). Saves ~"+Math.round(bhTime)+" min. BPH goes from "+predictedBph+" to <b style='color:#32D74B'>"+altBph+"</b></p></div>";
    }
  }
  document.getElementById("routeSuggestion").innerHTML=suggestion;
  ...
  var rhtml=fullRoute.map(function(r,idx){
    ...
    if(r.blackHole)tags+=" <span style='color:#FF9F0A'>BLACK HOLE</span>";
    else if(r.messy)tags+=" <span style='color:#FF9F0A'>CHURN</span>";
    var timeCol=r.predicted<=4?"#32D74B":r.predicted<=8?"#FF9F0A":"#FF453A";
    return "<div class='bay-item"+(r.overCap?" urgent":"")+(r.blackHole?" blackhole":"")+"'><div><div class='bay-name'><span style='color:#FF9F0A;margin-right:6px'>#"+(idx+1)+"</span>"+r.bay.name+tags+"</div><div style='color:#32D74B;font-size:12px;font-weight:700'>"+r.bikes+" bikes | <span style='color:"+timeCol+"'>~"+r.predicted+" min</span></div><div class='bay-dist'>"+fmtDist(r.dist)+" | ~"+r.walkMin+" min walk"+(idx===0?" from you":" from #"+idx)+"</div></div><div class='bay-actions'><a href='"+cm+"' target='_blank' style='background:#3DB551;color:#fff'>Go</a></div></div>";
  }).join("");
...
function osrmRoute(coords,callback){
  ...
  fetch("https://router.project-osrm.org/route/v1/foot/"+pts+"?overview=full&geometries=geojson")
    ...
    .then(function(data){if(data.routes&&data.routes[0])callback(data.routes[0].geometry.coordinates.map(function(c){return[c[1],c[0]]}));else callback(coords)})
...
function endShift(){
  ...
  var bayTimes=[];
  bays.filter(function(b){return b.ts&&b.ts!=="skipped"}).forEach(function(b){
    var ld=learnData[b.name];
    if(ld&&ld.visits.length>0){
      var last=ld.visits[ld.visits.length-1];
      bayTimes.push({name:b.name,time:last.dur,bikes:last.bikes});
    }
  });
  var fastest=bayTimes.length>0?bayTimes.reduce(function(a,b){return a.time<b.time?a:b}):{name:"N/A",time:0};
  var slowest=bayTimes.length>0?bayTimes.reduce(function(a,b){return a.time>b.time?a:b}):{name:"N/A",time:0};
  var entry={date:new Date().toLocaleDateString("en-GB"),zone:curZone==="bre1"?"BRE1":"WM",done:done,total:bays.length,hours:h.toFixed(1),bph:bph,fastest:fastest.name+" ("+fastest.time+"min)",slowest:slowest.name+" ("+slowest.time+"min)",bayTimes:bayTimes};
...
function showHistory(){
  ...
  document.getElementById("histList").innerHTML=hist.map(function(h){
    ...
    return "<div class='bay-item'><div><div class='bay-name'>"+h.date+" - "+h.zone+"</div><div class='bay-dist'>"+h.done+"/"+h.total+" | "+h.hours+"hrs | <span style='color:"+col+";font-weight:700'>"+h.bph+" bph</span></div><div style='color:#888;font-size:10px;margin-top:2px'>Fastest: "+h.fastest+" | Slowest: "+h.slowest+"</div></div></div>";
  }).join("");
  oP("histPanel");
}
...
function showInsights(){
  var done=bays.filter(function(b){return b.ts}).length;
  if(!done){toast("Tidy some bays first");return}
  var h=ss?(Date.now()-ss)/3600000:0;
  var bph=h>0.01?(done/h).toFixed(1):"0";
  var bayTimes=[];
  bays.filter(function(b){return b.ts&&b.ts!=="skipped"}).forEach(function(b){
    var ld=learnData[b.name];
    if(ld&&ld.visits.length>0){var last=ld.visits[ld.visits.length-1];bayTimes.push({name:b.name,time:last.dur,bikes:last.bikes})}
  });
  var html2="<div class='insight'><h4>Current Shift</h4><p>"+done+" bays in "+h.toFixed(1)+"hrs = <b style='color:"+(parseFloat(bph)>=6?"#32D74B":"#FF453A")+"'>"+bph+" bph</b></p></div>";
  if(bayTimes.length>0){
    bayTimes.sort(function(a,b){return a.time-b.time});
    html2+="<div class='insight'><h4>Fastest Bays</h4>";
    bayTimes.slice(0,5).forEach(function(bt){html2+="<p style='color:#32D74B'>"+bt.name+" — "+bt.time+" min ("+bt.bikes+" bikes)</p>"});
    html2+="</div>";
    html2+="<div class='insight'><h4>Slowest Bays</h4>";
    bayTimes.slice(-3).reverse().forEach(function(bt){html2+="<p style='color:#FF453A'>"+bt.name+" — "+bt.time+" min ("+bt.bikes+" bikes)</p>"});
    html2+="</div>";
  }
  var bhs=Object.keys(bikeChurn).filter(function(k){return bikeChurn[k].refills>=2});
  if(bhs.length>0){
    html2+="<div class='insight'><h4>Black Holes (skip these)</h4>";
    bhs.forEach(function(k){html2+="<p style='color:#FF9F0A'>"+k+" — refilled "+bikeChurn[k].refills+" times after tidy</p>"});
    html2+="</div>";
  }
  var totalLearn=Object.keys(learnData).length;
  html2+="<div class='insight'><h4>Learning Model</h4><p>"+totalLearn+" bays with timing data. Predictions improve each visit.</p></div>";
  document.getElementById("insightList").innerHTML=html2;
  oP("insightPanel");
}
